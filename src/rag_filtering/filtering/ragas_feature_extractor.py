...
Task
----
Given ``question``, ``context`` and ``answer`` (and optionally a gold answer),
...
Required raw columns: ``id``, ``question``, ``answer``, ``context``.
Optional: ``label``, ``gold_ans`` / ``gold_answer`` / ``reference`` / ...

Notes
-----
This module only creates features. Use ``RagasFilterTrainer`` for training and
``RagasFilter`` for inference / evaluation.
"""
...
class RagasFeatureExtractor:
    ...
    Reusable for both training and inference. ``ragas_evaluator`` is usually
    the :class:`rag_filtering.evaluation.ragas_wrapper.RAGAS` wrapper.
    """
    ...
    def validate_schema(self, df: pd.DataFrame) -> None:
        ...
        if missing:
            raise ValueError(
                ...
                f"Expected at least {RAW_REQUIRED_COLS}. "
    ...
    def _extract_ground_truths(self, df: pd.DataFrame) -> Optional[List[str]]:
        """Return gold answers as a list of strings if a gold column exists."""
        gold_col = _get_gold_col(df)
        if gold_col is None:
            logger.info(
                "No gold answer column found; ground-truth RAGAS metrics "
                "(answer_correctness, answer_similarity, context_recall) "
                "may be unavailable."
            )
            return None
        return df[gold_col].fillna("").astype(str).tolist()
    ...
    def transform(
        ...
        ground_truths = self._extract_ground_truths(df)

        if checkpoint_path is not None and hasattr(
            self.ragas_evaluator, "evaluate_checkpoint"
        ):
        ...
        if not kept_features:
            kept_features = ragas_df.select_dtypes(
                include=[np.number]
            ).columns.tolist()
            logger.warning(
                "No requested RAGAS cols found. Falling back to numeric cols: %s",
                kept_features,
            )
        if not kept_features:
            raise ValueError(
                "RAGAS result does not contain usable numeric feature columns."
            )
        ...
        if self.include_raw_columns:
            ...
            out = pd.concat(
                [df[raw_cols].reset_index(drop=True), feature_df], axis=1
            )
            out["parsed_context"] = df["_parsed_context"].apply(
                lambda x: json.dumps(x, ensure_ascii=False)
            )
        else:
            out = pd.concat(
                [df[[self.id_col]].reset_index(drop=True), feature_df], axis=1
            )
...
def build_ragas_features(
    ...
    """Convenience wrapper: build RAGAS features from a data file."""
