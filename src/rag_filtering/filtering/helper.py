...
Shared utilities for the RAGAS-feature-based faithfulness filter.

Provides:
- canonical feature/column constants
- robust context parsing (list / stringified list / JSON / dict / plain string)
- column-alias normalization into the canonical schema
- the default sklearn model zoo for the filter trainer
"""
...
def _normalize_col_aliases(df: pd.DataFrame) -> pd.DataFrame:
    """Normalize common column aliases into the canonical schema."""
    df = df.copy()
    alias_map = {
        "qid": "id",
        "sample_id": "id",
...
def parse_context(context_raw: Any) -> List[str]:
    """Robustly parse a context field into ``list[str]``.
...
def get_default_models(random_state: int = 42) -> Dict[str, BaseEstimator]:
    """Return the default sklearn model zoo used by the filter trainer."""
