...
Task
----
Train classical ML classifiers on RAGAS feature columns to predict whether an
answer is grounded in / supported by the context.

Input
-----
- ``feature_df`` / ``feature_path`` produced by :class:`RagasFeatureExtractor`
- a ``label`` column

Output
------
- best model saved as ``.joblib`` (bundled with feature columns)
- ``training_results.csv``, ``test_predictions.csv``, ``training_metadata.json``
...
Notes
-----
This module does NOT call RAGAS. Use :class:`RagasFeatureExtractor` first to
...
class RagasFilterTrainer:
    """Train sklearn classifiers from a RAGAS feature DataFrame / file."""
    ...
    def split_data(self, df: pd.DataFrame, feature_cols: Sequence[str]):
        ...
        stratify = (
            y if y.nunique() == 2 and y.value_counts().min() >= 2 else None
        )
    ...
    def train_all(self, df: Optional[pd.DataFrame] = None) -> TrainResult:
        ...
        for name, model in self.models.items():
            ...
            pred_df = pd.DataFrame(
                {"model": name, "y_true": y_test.values, "y_pred": preds}
            )
        ...
        results_df = (
            ...
            .sort_values([self.sort_by, "accuracy"], ascending=False)
    ...
    def save_reports(self, train_result: TrainResult) -> Dict[str, Path]:
        ...
        train_result.results_df.to_csv(
        ...
        train_result.test_predictions.to_csv(
...
def train_ragas_filter(
    ...
    """Convenience wrapper: train the RAGAS filter from a feature file."""
