...
def _truncation_collision_diagnostic(
    ...
    """
    # Build a base-id index (supports asqa_X_hallu and legacy asqa_Xb)
    from .data_split import to_base_id

    df_indexed = df.copy()
    df_indexed["_base"] = df_indexed["id"].map(to_base_id)
    df_indexed["_pos"] = list(range(len(df_indexed)))

    pair_rows = []
    for _base, group in df_indexed.groupby("_base"):
        if len(group) < 2:
            continue
    ...
    if not pair_rows:
        logger.warning(
            "Truncation diagnostic: no paired (correct, hallu) samples found"
        )
        return 0.0
...
def overfit_sanity_check(
    ...
    #
    # ROOT-CAUSE FIX (NaN explosion + underfit): the gate uses the SAME
    # differential optimizer as train_classifier. The pretrained backbone gets
    # a small LR + a LARGE eps (SGD-like, stable -> no NaN); the from-scratch
    ...
    # GATE backbone LR: SGD-like (eps=1.0) tolerates a higher LR than full
    # training because updates are gradient-proportional (no feature collapse).
    backbone_lr = float(cfg.get("overfit_backbone_lr", 1e-2))
    ...
    df_idx = df.copy()
    df_idx["_base"] = df_idx["id"].map(to_base_id)
    ...
    logger.info(
        "OVERFIT TEST: %d samples (%d pos, %d neg) for %d epochs at "
        "head_lr=%.0e (eps=%.0e) / backbone_lr=%.0e (eps=%.0e), batch_size=%d, "
        "max_length=%d, using HF Trainer (same stack as train_classifier)",
        len(mini_df),
        sum(1 for lbl in labels if lbl == 1),
        sum(1 for lbl in labels if lbl == 0),
        epochs, learning_rate, head_eps, backbone_lr, backbone_eps,
...
def _build_differential_optimizer(
    ...
    # 5.0; it is killed below via fused=False, foreach=False. The epsilons set
    # the optimizer REGIME per group:
    #   - backbone: LARGE eps (~1.0) -> gradient-PROPORTIONAL (SGD-like). A small
    #     Adam-like eps makes the update ~ +/-lr for every weight including the
    #     ~184M near-zero-gradient ones, which injects noise and collapses the
    #     MNLI features (tested: eps=1e-6 -> all logits ~0.5). Proportional
    #     updates move only what matters, so adapt the backbone via its LR, not
    #     a smaller eps.
    ...
    optimizer_grouped_parameters = [
        ...
        {"params": groups["backbone_nodecay"], "lr": backbone_lr,
         "weight_decay": 0.0, "eps": backbone_eps},
        {"params": groups["head_decay"], "lr": head_lr,
         "weight_decay": weight_decay, "eps": head_eps},
    ...
    logger.info(
        "Differential LR active: backbone=%.0e (eps=%.0e) head=%.0e (eps=%.0e) "
        "(warmup_steps=%d / total_steps=%d)",
        backbone_lr, backbone_eps, head_lr, head_eps,
