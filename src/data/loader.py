...
class ASQALoader:
    """
    Loader for ASQA (Answer Summaries for Questions which are Ambiguous) dataset in CSV format.
    
    ASQA is a long-form question answering dataset focused on ambiguous factoid questions.
    
    Expected CSV format (transformed from original ASQA):
        - id: str
        - question: str (ambiguous question)
        - answer: str (long-form answer, paragraph-length)
        - context: JSON string with {"title": [...], "sentences": [[...], [...]]}
        - supporting_facts: JSON string with {"title": [...]}
    """
    
    def __init__(self):
        """Initialize the ASQA data loader."""
        self.df_train: Optional[pd.DataFrame] = None
    ...
    @df_valid.setter
    def df_valid(self, value):
        """Setter for df_valid alias."""
        self.df_dev = value
    ...
    def load_data(
        ...
        Args:
            train_path: Path to training CSV file
            dev_path: Path to dev CSV file
            max_train_samples: Maximum number of training samples (None = all)
            max_dev_samples: Maximum number of dev samples (None = all)
            
        Returns:
            Tuple of (train_df, dev_df)
        """
        print(f"Loading ASQA data...")
        ...
        # Load and parse dev data
        self.df_dev = self._parse_csv(dev_path)
        if max_dev_samples:
            self.df_dev = self.df_dev.head(max_dev_samples)
        
        print(f"Loaded {len(self.df_train)} training samples")
        print(f"Loaded {len(self.df_dev)} dev samples")
    ...
    def build_corpus(
        ...
        # Deduplicate by title to avoid redundant passages
        unique_corpus = {}
        for doc in corpus:
            title = doc["title"]
        ...
        print(f"Built corpus: {len(self.corpus_texts)} unique passages from {len(df)} questions")
    ...
    def create_generator_examples(
        ...
        Each example consists of:
        - question: The ambiguous query
        - answer: The target long-form answer
        - contexts: Pre-computed contexts (optional)
    ...
    def get_statistics(self) -> dict:
        ...
        if self.df_train is not None:
            stats["train_samples"] = len(self.df_train)
            stats["train_avg_docs_per_question"] = self.df_train["docs"].apply(len).mean()
            stats["train_avg_answer_length"] = self.df_train["answer"].str.split().str.len().mean()
        
        if self.df_dev is not None:
            stats["dev_samples"] = len(self.df_dev)
            stats["dev_avg_docs_per_question"] = self.df_dev["docs"].apply(len).mean()
            stats["dev_avg_answer_length"] = self.df_dev["answer"].str.split().str.len().mean()
            # Also add as 'valid' for compatibility
            stats["valid_samples"] = len(self.df_dev)
            stats["valid_avg_docs_per_question"] = self.df_dev["docs"].apply(len).mean()
