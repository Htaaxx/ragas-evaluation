...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Self-RAG-style generative answer verifier",
    )
    parser.add_argument(
        "--config",
        default="rag_verifier",
        help="Config name in rag-interference/src/configs/",
    )
    parser.add_argument(
        "--csv",
        default=None,
...
def main() -> None:
    ...
    verifier = VerifierSystem(config_name=args.config)
    ...
    if args.evaluate:
        ...
        print(f"Evaluation complete: F1={metrics['f1']:.3f}, FPR={metrics['fpr']:.3f}")
